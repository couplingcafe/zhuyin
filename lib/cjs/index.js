...
const fromPinyinSyllable = (pinyin) => {
    let zy = py2zy_1.default[(0, pinyin_utils_1.removeTone)(pinyin).toLowerCase()];
    return zy + exports.toneMarks[(0, pinyin_utils_1.getToneNumber)(pinyin) - 1];
};
exports.fromPinyinSyllable = fromPinyinSyllable;
const fromPinyin = (input, everything = false) => {
    const translate = (pinyin) => {
        return (0, pinyin_split_1.default)(pinyin, everything).map((item) => {
            if (everything) {
                if (typeof item === "string") {
                    const zhuyin = (0, exports.fromPinyinSyllable)(item);
                    if (zhuyin && !zhuyin.includes("undefined")) {
                        return zhuyin;
                    }
                    return item;
                }
                else {
                    return (0, exports.fromPinyinSyllable)(item[0]);
                }
            }
            else {
                return (0, exports.fromPinyinSyllable)(item);
            }
        });
    };
...
const toPinyin = (zhuyin, opts = {}) => {
    let list = (0, exports.splitZhuyin)(zhuyin, opts.everything);
    if (!opts.everything)
    ...
    list = list.map((item) => {
        if (opts.everything && typeof item === "string")
            ...
            item = item[0];
        const pinyin = (0, exports.toPinyinSyllable)(item);
        if (opts.numbered)
            return opts.everything ? [pinyin] : pinyin;
        else if (opts.everything)
            return [(0, pinyin_utils_1.numberToMark)(pinyin)];
        else
            return (0, pinyin_utils_1.numberToMark)(pinyin);
    });
    return list;
